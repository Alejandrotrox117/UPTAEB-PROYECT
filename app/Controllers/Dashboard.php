...
/**
 * Controlador Dashboard - Estilo Funcional
 */
...
// Helpers locales (reemplazan métodos privados)

function dashboard_validarYFormatearFecha($dateString, $type = "inicio")
{
    if ($dateString) {
...
function dashboard_validarRangoFechas($desde, $hasta)
{
    return $desde <= $hasta;
}
...
function dashboard_getDashboardData()
{
    ...
    $fecha_desde_egresos = dashboard_validarYFormatearFecha($_GET["fecha_desde_egresos"] ?? null, "inicio");
    $fecha_hasta_egresos = dashboard_validarYFormatearFecha($_GET["fecha_hasta_egresos"] ?? null, "fin");
    ...
    if (
        !dashboard_validarRangoFechas($fecha_desde_ingresos, $fecha_hasta_ingresos) ||
        !dashboard_validarRangoFechas($fecha_desde_egresos, $fecha_hasta_egresos)
    ) {
...
function dashboard_getDashboardAvanzado()
{
    ...
    if (!dashboard_validarRangoFechas($prod_fecha_desde, $prod_fecha_hasta)) {
        http_response_code(400);
    ...
    try {
        ...
        error_log("Error en dashboard_getDashboardAvanzado: " . $e->getMessage());
...
function dashboard_getReportesSemanalesProduccion()
{
    ...
    if (!dashboard_validarRangoFechas($fecha_desde, $fecha_hasta)) {
        http_response_code(400);
    ...
    try {
        ...
        error_log("Error en dashboard_getReportesSemanalesProduccion: " . $e->getMessage());
        http_response_code(500);
        echo json_encode(["error" => "Error interno del servidor"]);
    }
    exit();
