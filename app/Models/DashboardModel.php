...
class DashboardModel extends Mysql
{
    ...
    public function getTopClientes($limit = 10)
    {
        ...
        $db = $conexion->get_conectGeneral();
        try {
            $limit = max(1, min(100, (int)$limit)); // Validar límite entre 1 y 100
            $stmt = $db->prepare("SELECT 
                c.idcliente,
    ...
    public function getTopProveedores($limit = 10)
    {
        ...
        try {
            ...
            $stmt = $db->prepare("SELECT 
                pr.idproveedor, 
                CONCAT(pr.nombre, ' ', COALESCE(pr.apellido, '')) as proveedor_nombre, 
                ...
                GROUP BY pr.idproveedor, pr.nombre, pr.apellido 
                HAVING total_comprado > 0 
                ORDER BY total_comprado DESC 
                LIMIT " . $limit);
    ...
    public function getKPIsEjecutivos()
    {
        ...
        try {
            ...
            $ingresos = $finanzas['ingresos_mes'] ?? 0;
            $egresos = $finanzas['egresos_mes'] ?? 0;
            $margen_ganancia = $ingresos > 0 ? (($ingresos - $egresos) / $ingresos) * 100 : 0;
            $roi_mes = $egresos > 0 ? (($ingresos - $egresos) / $egresos) * 100 : 0;
            
            // Rotación de inventario (días promedio)
            $stmt = $db->prepare("SELECT COALESCE(AVG(DATEDIFF(CURDATE(), fecha_creacion)), 0) as dias_promedio 
                FROM movimientos_existencia 
                WHERE cantidad_salida > 0 
            ...
            return [
                ...
                'roi_mes' => round($roi_mes, 2),
                'rotacion_inventario' => round($rotacion_inventario, 0),
