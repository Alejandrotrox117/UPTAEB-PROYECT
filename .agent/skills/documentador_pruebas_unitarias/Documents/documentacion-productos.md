...
Para las pruebas de inserción, se utilizan productos válidos como "Cartón Corrugado" con unidad de medida en kilogramos, precio de 0.15 USD y categoría 1, así como "Aceite Mineral" medido en litros a 2.50 BS con categoría 2. También se prueba con un producto duplicado llamado "Producto Ya Existente", productos que generen errores de conexión y casos donde lastInsertId retorna cero simulando un fallo silencioso en la inserción. En las pruebas de actualización se utilizan datos válidos como el cambio de un producto con ID 5 a "Cartón Premium" con precio 0.20 USD, o el cambio de moneda de un producto con ID 10 a "Aceite Industrial" de 3.00 BS. Se prueban conflictos de nombre al intentar actualizar el producto con ID 7 con el nombre "Nombre Duplicado", productos con IDs inexistentes como 999999, casos con nombres vacíos y excepciones simuladas de fallo de conexión. Para las consultas se simulan conjuntos de datos con dos productos activos incluyendo "Cartón" y "Plástico", productos individuales en USD y BS con todos sus atributos, búsquedas con IDs inexistentes como 999999 y 0, conjuntos vacíos de datos y excepciones de base de datos. En las pruebas de eliminación se utilizan IDs existentes como 1 y 50 que representan productos como "Cartón Corrugado" y "Plástico Duro", IDs inexistentes como 999999 y 888888, y productos para verificar que la operación sea un soft-delete.

**SALIDAS ESPERADAS:**

En las inserciones exitosas, el sistema debe retornar un array con status true, un mensaje indicando que el producto fue registrado exitosamente y el ID del producto generado por la base de datos. Cuando hay duplicación de nombres, debe retornar status false con un mensaje indicando que el producto ya existe, sin generar un nuevo ID. Si lastInsertId retorna cero o hay excepciones, debe retornar status false con un mensaje descriptivo del error. En las actualizaciones exitosas, el sistema debe retornar status true con un mensaje confirmando la actualización y afectando una fila en la base de datos. Al intentar actualizar con un nombre en conflicto, debe retornar status false con un mensaje indicando que el nombre ya existe. Si el ID no existe en la base de datos, debe retornar status false ya que ninguna fila fue afectada. Las excepciones deben manejarse retornando status false con el mensaje de error correspondiente. Para las consultas, cuando hay datos disponibles el sistema debe retornar un array con status true, mensaje descriptivo y un array data conteniendo todos los productos con su estructura completa incluyendo ID, nombre, descripción, precio, unidad de medida, existencia, categoría, moneda y estatus. Al buscar un producto por ID existente debe retornar el array con todos los datos del producto. Si el ID no existe debe retornar false. Las consultas de productos activos y categorías activas deben retornar solo registros con estatus ACTIVO. Cuando no hay datos disponibles debe retornar arrays vacíos pero con status true. Las excepciones en consultas deben retornar status false con arrays data vacíos. En las eliminaciones, cuando el producto existe el sistema debe retornar true tras marcar el producto como INACTIVO mediante un UPDATE, no un DELETE físico. Al intentar eliminar un producto inexistente debe retornar false. Las excepciones deben manejarse retornando false. La prueba de soft-delete debe confirmar que ninguna query ejecutada contenga la instrucción DELETE FROM, garantizando que se usa UPDATE para cambiar el estatus.
...
Se ejecutaron exitosamente las 29 pruebas unitarias del módulo de Productos, todas ellas completándose satisfactoriamente con un total de 105 aserciones. Las pruebas cubrieron exhaustivamente las cuatro operaciones fundamentales del CRUD: inserción, actualización, consulta y eliminación de productos. Durante la validación de la inserción de productos se comprobó que el sistema maneja correctamente la creación de nuevos productos con diferentes configuraciones de moneda, unidades de medida y categorías, además de rechazar apropiadamente los intentos de registrar productos con nombres duplicados y gestionar adecuadamente las excepciones de base de datos. En las pruebas de actualización se verificó que el sistema permite modificar la información de productos existentes, incluyendo campos como precio, descripción y moneda, mientras mantiene la validación de unicidad de nombres y retorna errores descriptivos cuando se intenta actualizar productos inexistentes. Las operaciones de consulta demostraron funcionar correctamente tanto al obtener todos los productos como al filtrar por ID específico o estatus, manejando apropiadamente tanto conjuntos de datos poblados como vacíos, y respondiendo adecuadamente a búsquedas de registros inexistentes. La funcionalidad de eliminación confirmó que el sistema implementa correctamente un soft-delete mediante la actualización del campo estatus a INACTIVO en lugar de eliminar físicamente los registros de la base de datos, preservando así la integridad histórica de los datos. El conjunto de pruebas utilizó mocks de PDO para aislar completamente la lógica de negocio de la capa de persistencia, permitiendo validar el comportamiento del modelo sin depender de una base de datos real. La ejecución tomó aproximadamente 20 segundos utilizando 10 MB de memoria. Se reportó una advertencia menor sobre la ausencia de un driver de cobertura de código, lo cual no afecta la validez de las pruebas ejecutadas. Todos los escenarios de error fueron manejados correctamente, retornando los mensajes apropiados y manteniendo la estabilidad del sistema incluso ante excepciones de base de datos.
