## Cuadro Nº 1: Módulo de Bitácora (RF01)

### Objetivos de la prueba

Validar que el registro de acciones en la bitácora del sistema solo se ejecute cuando los datos de entrada sean completos y válidos (tabla, acción e ID de usuario). El sistema debe rechazar registros sin usuario, sin módulo, sin acción o con IDs negativos, y garantizar que las consultas de historial retornen resultados correctos según los filtros aplicados.

### Técnicas

Pruebas de caja blanca con enfoque en aislamiento total de la base de datos mediante mocks de PDO (Mockery con `overload:App\Core\Conexion`). Se evalúan los métodos `registrarAccion()`, `obtenerHistorial()`, `SelectAllBitacora()` y `obtenerRegistroPorId()` en escenarios válidos e inválidos, verificando retorno de IDs insertados, arrays de registros y valores `false` según corresponda.
...
**OBJETIVO:** Verificar que `BitacoraModel` registre, consulte y recupere entradas de auditoría de manera correcta, rechazando datos inválidos y retornando resultados precisos según los filtros aplicados.

**DESCRIPCIÓN:** Se prueban los cuatro métodos principales del modelo: `registrarAccion()` con entradas válidas e inválidas, `obtenerHistorial()` con y sin filtros, `SelectAllBitacora()` con y sin datos, y `obtenerRegistroPorId()` con IDs existentes, inexistentes y fuera de rango.

**ENTRADAS:**
- `registrarAccion('PRODUCTOS', 'INSERT', 1, 'Registro de prueba')` → caso válido, ID insertado `'42'`
- `registrarAccion('VENTAS', 'UPDATE', 5, 'Actualización', 123)` → caso válido con idRegistro, ID `'99'`
- `registrarAccion('', 'INSERT', 1, ...)` / `registrarAccion('PRODUCTOS', '', 1, ...)` → tabla o acción vacía
- `registrarAccion('PRODUCTOS', 'INSERT', null, ...)` / `registrarAccion(..., -1, ...)` → sin usuario o ID negativo
- `obtenerRegistroPorId(0)`, `obtenerRegistroPorId(-1)`, `obtenerRegistroPorId(999999)` → IDs inválidos o inexistentes

**SALIDAS ESPERADAS:**

| Escenario | Resultado esperado |
|---|---|
| `registrarAccion` con datos válidos | Retorna el ID insertado (`'42'`, `'99'`) |
| `registrarAccion` sin usuario / tabla / acción vacíos | Retorna `false` |
| `registrarAccion` con ID de usuario negativo | Retorna `false` |
| `registrarAccion` cuando `execute()` falla en BD | Retorna `false` |
| `obtenerHistorial([])` con registros disponibles | Retorna array con 1 elemento, tabla `'PRODUCTOS'` |
| `obtenerHistorial(['idusuario' => 7])` | Retorna array con registro filtrado por usuario |
| `obtenerHistorial` con usuario/módulo inexistente | Retorna array vacío |
| `obtenerRegistroPorId(5)` con ID existente | Retorna array con `idbitacora = 5` |
| `obtenerRegistroPorId(0/-1/999999)` | Retorna `false` |
...
Time: 00:03.910, Memory: 10.00 MB

Bitacora Unit (Tests\UnitTest\Bitacora\BitacoraUnit)
 ✔ RegistrarAccion Exitosa RetornaIdInsertado with insert_productos
 ✔ RegistrarAccion Exitosa RetornaIdInsertado with update_ventas_conId
 ✔ RegistrarAccion Falla RetornaFalse with sin_usuario
 ✔ RegistrarAccion Falla RetornaFalse with sin_modulo
 ✔ RegistrarAccion Falla RetornaFalse with sin_accion
 ✔ RegistrarAccion Falla RetornaFalse with id_negativo
 ✔ RegistrarAccion Falla CuandoEjecutaFallaEnBD
 ✔ ObtenerHistorial RetornaDatos CuandoExistenRegistros
 ✔ ObtenerHistorial FiltradoPorUsuario RetornaDatos
 ✔ ObtenerHistorial RetornaVacio CuandoUsuarioInexistente
 ✔ ObtenerHistorial RetornaVacio CuandoModuloInexistente
 ✔ SelectAllBitacora RetornaDatos CuandoExistenRegistros
 ✔ SelectAllBitacora RetornaArrayVacio SinRegistros
 ✔ ObtenerRegistroPorId RetornaDatos CuandoIdExiste
 ✔ ObtenerRegistroPorId RetornaFalse CuandoIdNoExiste
 ✔ ObtenerRegistroPorId RetornaFalse ConIdsInvalidos with id_cero
 ✔ ObtenerRegistroPorId RetornaFalse ConIdsInvalidos with id_negativo
 ✔ ObtenerRegistroPorId RetornaFalse ConIdsInvalidos with id_muy_alto
...
Se ejecutaron 18 pruebas con 28 aserciones en 3.910 segundos, todas con resultado exitoso. La prueba más destacada es la cobertura completa de `registrarAccion()`, que valida 4 escenarios de fallo por datos incompletos más un caso de fallo de base de datos. El uso de `#[RunTestsInSeparateProcesses]` con `overload:Conexion` garantiza aislamiento total sin afectar la base de datos real.
